...
extension View {
    ...
    /// large, or while `raised` holds. Interaction with the view behind the sheet is enabled up through the smaller
    /// detent.
    public func presentationDetents(fold: CGFloat?, otherwise: PresentationDetent = .medium, raised: Bool = false) -> some View {
        modifier(FoldDetents(fold: fold, otherwise: otherwise, raised: raised))
    }
}
...
private struct FoldDetents: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        content
            ...
            .onChange(of: raised) { _, raised in
                withAnimation { stand = raised ? .large : .smaller }
            }
    }
}
